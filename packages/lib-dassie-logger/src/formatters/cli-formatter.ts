...
import { isError, isObject } from '@nostream-ilp/lib-dassie-type-utils'
...
const formatFilePath = (filePath: string) => {
  ...
  const match = /^(.*)\/packages\/([\da-z-]+)\/(.*)$/.exec(filePath) as unknown as
...
const formatStack = (context: FormattingContext, stack: string): string => {
  ...
  const formattedCallsites = lines
    ...
    .map((line) => {
      {
        const match = /^ {4}at (.*) \((.*):(\d+):(\d+)\)$/.exec(line) as unknown as
