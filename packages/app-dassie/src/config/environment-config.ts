...
export function EnvironmentConfig(): Config {
  ...
  if (
    environment.DASSIE_RPC_AUTH_TOKEN &&
    environment.DASSIE_RPC_AUTH_TOKEN.length < 32
  ) {
    throw new Error(
  ...
  return {
    ...
    rpcAuthToken: environment.DASSIE_RPC_AUTH_TOKEN ?? false,
