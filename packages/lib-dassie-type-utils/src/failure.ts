 ...
 *
 * We are using a marker instead of instanceof to avoid issues with multiple
 * JavaScript realms or multiple copies of this library. We are using a string
 * instead of Symbol.for because TypeScript doesn't support symbol literals at
 * the time of writing.
 */
 ...
 * @remarks
 *
 * You can create a subclass to represent your various failure types. If your
 * failure doesn't have any parameters, you can also create a constant
 * containing an instance of this class to avoid creating a new instance every
 ...
 * @remarks
 *
 * To handle failures, you would first check if a result is a failure and then
 * handle different types by switching on the `name` property.
 *
 * @example
 *
 * ```ts
 ...
 * @remarks
 *
 * This is useful for checking if a list of results contains any failures. The
 * first failure that is found is returned. If no failures are found, the
 * original array is returned.
 */
export const findFailure = <T extends readonly unknown[]>(
  values: T,
): InferFindFailure<T> => {
  const firstFailure = values.find(isFailure as (
    value: unknown,
  ) => value is InferFindFailure<T>)
